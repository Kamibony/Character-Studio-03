...
// Helper funkcia na konverziu súboru na Base64 reťazec
const toBase64 = (file: File): Promise<string> => new Promise((resolve, reject) => {
    const reader = new FileReader();
    reader.readAsDataURL(file);
    reader.onload = () => {
        // Výsledok je formát "data:mime/type;base64,samotny_base64_retazec"
        // Potrebujeme odstrániť prefix
...
const FileUploadPage: React.FC = () => {
  ...
  const handleCreate = async () => {
    ...
    try {
      // 1. Konvertovať súbor na Base64
      const imageBase64 = await toBase64(file);

      // 2. Zavolať Cloud Function s Base64 dátami a názvom súboru
      const newCharacter = await api.createCharacterProfile(imageBase64, file.name);

      // 3. Presmerovať na stránku novej postavy
      ...
      <div className="bg-gray-800 p-8 rounded-lg shadow-lg">
        ...
        {error && (
            <div className="mt-4 p-4 text-sm text-red-300 bg-red-900/50 rounded-lg">
                <p className="font-bold">Chyba pri vytváraní</p>
                <p>{error}</p>
            </div>
        )}
